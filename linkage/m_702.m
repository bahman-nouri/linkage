...
c_qt=[x1+d/2*cos(t1);...
    y1+d/2*sin(t1);...
    %p2
    x1-d/2*cos(t1)-x2+a/2*cos(t2);...
    y1-d/2*sin(t1)-y2+a/2*sin(t2);...
    %p2
    x1-d/2*cos(t1)-x3+a/2*cos(t3);...
    y1-d/2*sin(t1)-y3+a/2*sin(t3);...
    %p6
    x5-b/2*cos(t5)-d;...
    y5-b/2*sin(t5);...
    %p6
    ...
    %p3
    x2+a/2*cos(t2)-x5-b/2*cos(t5);...
    y2+a/2*sin(t2)-y5-b/2*sin(t5);...
    %p4
    x4+a/2*cos(t4)-x6-a/2*cos(t6);...
    y4+a/2*sin(t4)-y6-a/2*sin(t6);...
    %p5
    x6-a/2*cos(t6)-x3-a/2*cos(t3);...
    y6-a/2*sin(t6)-y3-a/2*sin(t3);...
    %p5
    x3+a/2*cos(t3)-x7-b/2*cos(t7);...
    y3+a/2*sin(t3)-y7-b/2*sin(t7);...
    t1-w*t];
...
linkn(a,q0(4,1),q0(5,1),q0(6,1),'o','o');
linkn(a,q0(7,1),q0(8,1),q0(9,1));
[p1 p2]=linkn(a,q0(10,1),q0(11,1),q0(12,1),'o');
pivot4X(:,m)=p2(1,1);
pivot4Y(:,m)=p2(1,2);
linkn(b,q0(13,1),q0(14,1),q0(15,1));
linkn(a,q0(16,1),q0(17,1),q0(18,1));
linkn(b,q0(19,1),q0(20,1),q0(21,1),'o','^');
xlim([-3,10]);
ylim([-6,7]);
pause(n)
hold off
